...
class Signs:
    ...
    mode :: applies to backend 'gensim'. The options are 'glove', 'word2vec', and 'fasttext'
    temp :: filename for glove to word2vec conversion
    binary :: if the vector file format is binary or not
    
    '''
    
    def __init__(self, trained_vectors='en', backend='spacy', mode=None, temp='signs_temp.sig', binary=False):
    ...
    def load_model(self):
        ...
        if self.backend == 'spacy':
            ...
            if self.mode is 'glove':
                ...
                self.model = KeyedVectors.load_word2vec_format(self.temp, binary=self.binary)
                os.remove(self.temp)
                ...
                self.model = FastText.load_fasttext_format(self.trained_vectors, encoding='latin-1')
    ...
    def similarity(self, word1, word2):
        ...
        EXAMPLE
        -------
        get = Signs(backend='gensim', trained_vectors="e:/signs_dev/vectors/glove/glove.twitter.27B.25d.txt")
